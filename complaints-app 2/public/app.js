...
const api = {
  async list(){ return (await fetch("/api/complaints")).json(); },
  async create(b){ return (await fetch("/api/complaints",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(b)})).json(); },
  async update(id,b){ return (await fetch("/api/complaints/"+id,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(b)})).json(); },
  async remove(id){ return (await fetch("/api/complaints/"+id,{method:"DELETE"})).json(); },
  async import(records){ return (await fetch("/api/import",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(records)})).json(); }
};
...
(async function init(){
  ...
  // إعادة الفحص كل نصف ساعة للتذكير بالمواعيد
